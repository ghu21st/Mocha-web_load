#!/usr/localbin/perl
# Live Assist Sip + gui load test Sip log pass/fail summary result parsing script 
# usage: 
# perl parseSipTestlog.pl <target_Sip_load_test_result_files_dir> <target_each_chl_call_iteration>
# ------------------------------------------------------------

use File::Find;

$dir = $ARGV[0]; # point to test folder with Sip test logs
...
$pass_target_rate = $total_passed_calls/($target_iter * $total_ports_count); 
	...
	printf "\n\t total average Sip response time (sec): %.2f", $total_avg_Sip_response_time / $total_ports_count;
	printf "\n\t total Sip call load test elapse (hr): %.2f", $total_test_elapse/3600/$total_ports_count;
	print "\n\n";
...
sub call_summary
{
	...
	$channel_avg_Sip_response_time = 0;
	$channel_total_Sip_response_time = 0;
	...
	while(<FILE>)
	{
		...
		if (/Iteration Sip repsonse time = (\d+)/) {
			$channel_total_Sip_response_time = $channel_total_Sip_response_time + $1;
	...
	$channel_avg_Sip_response_time = $channel_total_Sip_response_time / ($calls_passed + 1);
	$total_avg_Sip_response_time = $total_avg_Sip_response_time + $channel_avg_Sip_response_time;
	...
	printf "\tSip call avg response time (sec): %.2f\n", $channel_avg_Sip_response_time;
	printf "\tSip call test elapse(sec): %.2f \n\n", $channel_test_elapse;
	print "###############################\n";
}
