//TC0005-webapi: Live Assist load test web api call script
//-------------------------------------------------------
var case_ID = '0005-webapi';
        ...
        async.series([
            ...
            function(callback){
                ...
                test_client.post2(testAPIURL + '/inputInteractionStep', testBodyParam, function(err, res, body){
                    ...
                    var body_ret = body;
                    //var body_ret = JSON.parse(body); //web
                    ...
                    function(cb){
                        ...
                        test_client.post2(testAPIURL + '/inputInteractionStep', testBodyParam, function(err, res, body){
                            if(err){
                                ...
                                //body_ret = body; //for ivr

                                body_ret = JSON.stringify(body); //for web
            ...
            function(err, result){
                ...
                if(err){
                    ...
                    //
                    var chk_res = 0;
                    var chk1_str = 'session:Nuance-WebApiCallSteering';
                    var chk2_str = 'agentPending';
                    var chk3_str = 'INTENT';
                    var chk4_str = 'WAIVE_FEE_REQUEST';

                    if((result[1][1].sessionID).indexOf(chk1_str) >= 0){
                        if((result[1][1].state).indexOf(chk2_str) >= 0){
