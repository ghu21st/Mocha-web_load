//TC0002: Live Assist load test script with simple app IVR api + Agent GUI
//Note: GUI with agent CHECK_BALANCE intent selection!
//-------------------------------------------------------
var case_ID = '0002-gui';
var agent_id = 'TA0002';
...
var ptor_timeout = TestConfig.options.ptor_timeout_load2;	//gui test timeout
...
var selenium_server = 'http://' + TestConfig.options.first_selenium_host + ':4442/wd/hub';
        ...
        async.series([
            ...
            function(callback){
                ...
                if(case_call_run > 0){
                    ...
                    }, ptor_page_timeout).then(function(b){ //old: 12000
                            if(b){
           ...
           function(callback){
                    //// GUI with agent CHECK_BALANCE intent selection!  ##############
            ...
            function(callback){
                ...
                if(case_call_run < (call_run_num - 1)){
                    ...
                    ptor.driver.wait(function(){
                        return ptor.isElementPresent(protractor.By.id('signOutButton'));
                    }, ptor_page_timeout).then(function(b){ //old 120000
                            if(b){
                                output_msg = '\nFound the new agent web page with specific className';//for debugging
                                output_string += output_msg;
                                logger.info(output_msg);
