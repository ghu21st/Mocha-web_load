//TC0001: Live Assist load test script with simple app IVR api + Agent GUI
//Note: GUI with agent BOOK_FLIGHT (boston) intent selection!
//-------------------------------------------------------
var case_ID = '0001-gui';
var agent_id = 'TA0001';
...
var case_call_run = -1; //total call iteration # for this channel
...
*/
				...
				var selenium_server = 'http://' + TestConfig.options.first_selenium_host + ':' + TestConfig.options.selenium_user_port[agent_id] + '/wd/hub';
		/*
				if(TestConfig.options.browser_driver == "chrome"){
	                ret = ptorDriver.createPtorDriver(selenium_server, TestConfig.options.browser_driver, TestConfig.options.user_profile_location[agent_id]);
				}else if (TestConfig.options.browser_driver == "firefox"){
	                ret = ptorDriver.createPtorDriver(selenium_server, TestConfig.options.browser_driver, agent_id);
				}else{ 					//wrong browser name assigned
					logger.info('\nWrong browser name assignment!! Please check the configuration file.\n\n');
					process.exit(1);
				}
		*/
                ret = ptorDriver.createPtorDriver(selenium_server, TestConfig.options.browser_driver, agent_id);
				...
				//for debug:
				logger.info('Chrome user profile location used for this test: \n' + ret[2]);
                //
...
async.whilst(
    function(){
		return case_call_run < call_run_num - 1;  
	},
    function(callback0){
        //------------------------------------------
        ...
        async.series([
           ...
           function(callback){	//BOOK_FLIGHT intent
                    //GUI with agent BOOK_FLIGHT (boston) intent selection!
                    ptor.actions().sendKeys(protractor.Key.chord(protractor.Key.CONTROL, "3"), protractor.Key.NULL).perform().then(function(){
                    ...
                    ptor.driver.wait(function(){
                        return ptor.isElementPresent(protractor.By.id('signOutButton'));
                            ...
                            if(b){
					            ...
					            //keyboard
								element1.sendKeys(protractor.Key.ENTER).then(function(){
									output_msg = '\Sign out botton pressed';
									output_string += output_msg;
									logger.info(output_msg);
									//
									callback(null, 13);
