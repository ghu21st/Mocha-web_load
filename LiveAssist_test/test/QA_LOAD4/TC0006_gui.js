//TC0006: Live Assist load test script with simple app IVR api + Agent GUI
//Note: GUI with agent UPDATE_CONTACT_INFORMATION intent selection!
//-------------------------------------------------------
var case_ID = '0006-gui';
var agent_id = 'TA0006';
...
var selenium_server = 'http://' + TestConfig.options.second_selenium_host + ':4446/wd/hub';
...
*/
				...
				var selenium_server = 'http://' + TestConfig.options.second_selenium_host + ':' + TestConfig.options.selenium_user_port[agent_id] + '/wd/hub';
...
async.whilst(
    ...
    function(callback0){
        ...
        async.series([
           ...
           function(callback){
				//// GUI with agent UPDATE_CONTACT_INFORMATION intent selection!  ##############
				ptor.actions().sendKeys(protractor.Key.chord(protractor.Key.CONTROL, "2"), protractor.Key.NULL).perform().then(function(){
          ...
          //wait for 1 sec
			...
			function(callback){
				...
				ptor.driver.switchTo().activeElement().sendKeys('').then(function(){
					ptor.driver.sleep(1000);
					ptor.driver.switchTo().activeElement().sendKeys('TEST').then(function(){
						ptor.driver.sleep(1000);
						var result = ['TEST1', 13];
						callback(null, result);
					});
					...
					ptor.element.all(protractor.By.repeater('variable in selectedIntents[selectedIntents.length - 1].variables')).then(function(arr){
                        ...
                        arr[0].evaluate('variable.id').then(function(var_id) {
                            arr[0].evaluate('variable.name').then(function (var_name) {
								...
								ptor.actions().sendKeys(protractor.Key.TAB).perform().then(function(){
								    ptor.driver.sleep(500);
                                    element3 = ptor.element.all(protractor.By.id(var_id)).get(0);
                                    element3.clear().then(function(){
					                    ptor.driver.sleep(500);
                                        element3.sendKeys('QA_TEST1').then(function (){
						                    ptor.driver.sleep(500);
											  var result = [var_id_res, var_name_res];
                                              callback(null, result);
