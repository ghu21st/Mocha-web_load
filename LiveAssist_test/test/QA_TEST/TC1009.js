/**
 TC1009: Verify that Live Assist IVR VXML driven API method /getLiveAssistResult
 **/
//QA test case description, verification & assertion check
describe("TC1009: Verify that Live Assist IVR VXML driven API method /getLiveAssistResult with canStreamAudio=false", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1009: Verify that Live Assist IVR VXML driven API method /getLiveAssistResult with canStreamAudio=false", function (done) {
        //start async series task ~~~~~~~~~~~~~~~~
        var testBaseURL = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/rest/vxmlapp';
        ...
        async.series([
            ...
            function(callback){
                //
                var testBodyParam = 'varName=Mycroft&varVersion=1.0&ani=sip:1111@QA_test_server&dnis=sip:1111@QA_test_server&externalSessionID=0a0325e7_00004175_528d983c_0055_0001&canStreamAudio=false';
            ...
            function(callback){
                //
                var testBodyParam = 'sessionID=' + sessionID + '&pickList=speak+to+an+agent%7Cchange+my+tariff%7Ctechnical+support%7Coperator';
                test_client.post(testBaseURL + '/startInteraction', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http post request on /startInteraction' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 2);
                    }
                    output_string += '\n\nDebug: send post request \'\/startInteraction\' done\n\n';//for debugging
                    output_string += '\npost test body param: ' + testBodyParam; //for debugging
                    output_string += '\nResponse: ' + res;//for debugging
                    output_string += '\nBody: ' + body;//for debugging
            ...
            function(callback){
                //
                var testBodyParam = "sessionID=" + sessionID + "&recognitionResult=<?xml version='1.0'?><emma:emma version='1.0' xmlns:emma='http://www.w3.org/TR/2007/CR-emma-20071211' xmlns:nuance='http://nr10.nuance.com/emma'><emma:grammar id='grammar_1' ref='session:http://10.3.9.82:8080/liveassist/whatdoyouwant.grxml -1 -1 10000'/><emma:interpretation id='interp_1' emma:uninterpreted='true' emma:confidence='0.31' emma:grammar-ref='grammar_1' emma:tokens='operator' emma:duration='1220' nuance:eos-silence='1240'/></emma:emma>&utterance=http://mtl-da53:90/Nuance/callLogs/Test2/2013/11November/21/00/NUAN-21-01-mtl-da53-0a0325e7_00004175_528d983c_0055_0001-utt001-SAVEWAVEFORM.wav";

                test_client.post(testBaseURL + '/sendInteractionResult', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http post request on /sendInteractionResult' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                        callback(err, 3);
                    }
                    output_string += '\n\nDebug: send post request \'\/sendInteractionResult\' done\n\n';//for debugging
                    output_string += '\npost test body param: ' + testBodyParam; //for debugging
                    output_string += '\nResponse: ' + res;//for debugging
                    output_string += '\nBody: ' + body;//for debugging
            ...
            function(callback){
                ...
                test_client.post(testBaseURL + '/getLiveAssistResult', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http post request on /startSession' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
                    ...
                    output_string += '\n\nDebug: send post request \'\/getLiveAssistResult\' done\n\n';//for debugging
                    output_string += '\npost test body param: ' + testBodyParam; //for debugging
                    output_string += '\nResponse: ' + res;//for debugging
                    output_string += '\nBody: ' + body;//for debugging
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[2][1]).to.match(/<\?xml.+\?>/)
                        .and.to.match(/<startInteractionResult>/)
                        .and.to.match(/<interactionID>.+<\/interactionID>/)
                        .and.to.match(/<\/startInteractionResult>/);

                    expect(result[3][0].statusCode).to.eql(200);
                    expect(result[3][1]).to.match(/<\?xml.+\?>/)
                        .and.to.match(/<sendInteractionResult\/>/);

                    expect(result[4][0].statusCode).to.eql(200);
                    expect(result[4][1]).to.match(/<\?xml.+\?>/)
                        .and.to.match(/<getLiveAssistResult>/)
                        .and.to.match(/<state>agentPending<\/state>/)
                        .and.to.match(/<\/getLiveAssistResult>/);
