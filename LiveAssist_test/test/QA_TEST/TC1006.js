/**
 TC1006: Verify that session storage debug API get sessions and basic session storage info
 **/
//QA test case description, verification & assertion check
describe("TC1006: Verify that session storage debug API get sessions and basic session storage info", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1006: Verify that session storage debug API get sessions and basic session storage info", function (done) {
        //start async series task ~~~~~~~~~~~~~~~~
        var testBaseURL = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/rest/debug';
        var testBaseURL2 = 'http://' + TEST_HOST + ':' + TEST_PORT + '/liveassist/rest/vxmlapp';
        var sessions = []; //session array for assertion check with get session method
        ...
        async.series([
            ...
            function(callback){
                ...
                test_client.delete(testBaseURL + '/sessions', function(err, res, body){
                    if(err){
            ...
            function(callback){
                ...
                test_client.get(testBaseURL + '/sessions', function(err, res, body){
                    if(err){
            ...
            function(callback){
                //start a new session by post
                var testBodyParam = 'varName=Mycroft&varVersion=1.0&ani=sip:1111@QA_test_server&dnis=sip:1111@QA_test_server&externalSessionID=0a0325e7_00004175_528d983c_0055_0001';
                test_client.post(testBaseURL2 + '/startSession', testBodyParam, function(err, res, body){
                    if(err){
                    ...
                    output_string += '\nBody: ' + body;//for debugging
                    //
                    var session = body.match(/sessionID>(.+)<\/sessionID/)[1]; //pattern search found the real sessionID #
                    sessions.push(session);  //sessions array

                    var msg = '\nSession from /startSession is: ' + session + '\n';
                    console.log(msg);
                    output_string += msg;
            ...
            function(callback){
                //start a new session by post
                //note: customized QA post "varName=Mycroft&varVersion=1.0&ani=sip:2222@QA_test_server&dnis=sip:2222@QA_test_server&externalSessionID=0a0325e7_00004175_528d983c_0055_0001"
                var testBodyParam = 'varName=Mycroft&varVersion=1.0&ani=sip:2222@QA_test_server&dnis=sip:2222@QA_test_server&externalSessionID=0a0325e7_00004175_528d983c_0055_0001';
                test_client.post(testBaseURL2 + '/startSession', testBodyParam, function(err, res, body){
                    if(err){
                        err_msg='Error found from http post request on /startSession' + err;
                        err_array.push(err_msg);
            ...
            function(callback){
                //Debug API - get sessions, return all sessions info in an object
                test_client.get(testBaseURL + '/sessions', function(err, res, body){
                    if(err){
                        err_msg='Error found from http get request on /sessions' + err;
                        err_array.push(err_msg);
            ...
            function(err, result){
                ...
                //For JSON result and logging
                var ret_result = result[5][1].match(/\[(.+)]/)[1];
                console.log('\nret_result: ' + ret_result);
                //var ret_string = JSON.stringify(ret_result);

                //For debug and logging
                //console.log('\nret_string: ' + ret_string);
                //console.log('\nret_session1: ' + ret_session1);
               //console.log('\nret_session2: ' + ret_session2);
                console.log('\nrecorded sessions: \n' + sessions[0] + '\n' + sessions[1]);

                //using try & catch block to customize the output message if the case failed
                try{
                    //http response return assertion check
                    expect(result[1][0].statusCode).to.eql(204);
                    //expect(result[1][1]).to.match(/<\?xml.+\?>/);

                    expect(result[2][0].statusCode).to.eql(200);
                    expect(result[2][1]).to.match(/\[\]/);

                    expect(result[3][0].statusCode).to.eql(200);
                    expect(result[3][1]).to.match(/<\?xml.+\?>/)
                        .and.to.match(/<startSessionResult>/)
                        .and.to.match(/<sessionID>.+<\/sessionID>/)
                        .and.to.match(/<\/startSessionResult>/);

                    expect(result[4][0].statusCode).to.eql(200);
                    expect(result[4][1]).to.match(/<\?xml.+\?>/)
                        .and.to.match(/<startSessionResult>/)
                        .and.to.match(/<sessionID>.+<\/sessionID>/)
                        .and.to.match(/<\/startSessionResult>/);

                    //verify result session
                    //ret_result: "id":"session:Mycroft:dc17ae4a-4bfe-421b-b3c9-e017bf71317d","varName":"Mycroft","varVersion":"1.0",
                    // "ani":"sip:1111@QA_test_server","dnis":"sip:1111@QA_test_server","externalSessionID":"0a0325e7_00004175_528d983c_0055_0001","currentInteractionIndex":0,
                    // "currentInteractionId":null,"pickList":null,"recognitionResult":null,"utterance":null,"audioStream":null,"shouldEscalate":false,
                    // "escalated":false,"outcome":null,"sessionStats":{"stepTimes":{"CREATE":{"date":1387418856274},"START_SESSION":{"date":1387418856274}}},"viewName":null,
                    // "variableNames":null,"outcomeValues":null,"currentInteraction":null},{"id":"session:Mycroft:2369c6b5-b89a-43c5-96b7-bbc0a403b0bc","varName":"Mycroft",
                    // "varVersion":"1.0","ani":"sip:2222@QA_test_server","dnis":"sip:2222@QA_test_server","externalSessionID":"0a0325e7_00004175_528d983c_0055_0001",
                    // "currentInteractionIndex":0,"currentInteractionId":null,"pickList":null,"recognitionResult":null,"utterance":null,"audioStream":null,"shouldEscalate":false,"escalated":false,"outcome":null,"sessionStats":{"stepTimes":{"CREATE":{"date":1387418856281},"START_SESSION":{"date":1387418856281}}},"viewName":null,"variableNames":null,"outcomeValues":null,"currentInteraction":null

                    expect(ret_result).to.match(/\"id\"/)
                        .and.to.have.string(sessions[0])
                        .and.to.match(/\"varName\":\"Mycroft\"/)
                        .and.to.match(/\"varVersion\":\".+\"/)
                        .and.to.match(/\"ani\":\"sip:1111\@QA_test_server\"/)
                        .and.to.match(/\"dnis\":\"sip:1111\@QA_test_server\"/)
                        .and.to.match(/\"externalSessionID\":\"0a0325e7_00004175_528d983c_0055_0001\"/)
                        .and.to.match(/\"escalated\":false/)
                        .and.to.match(/\"sessionStats\":/)
                        .and.to.match(/\{\"stepTimes\":/)
                        .and.to.match(/\{\"CREATE\":/)
                        .and.to.match(/\"date\":.+/);

                    expect(ret_result).to.match(/\"id\"/)
                        .and.to.have.string(sessions[1])
                        .and.to.match(/\"varName\":\"Mycroft\"/)
                        .and.to.match(/\"varVersion\":\".+\"/)
                        .and.to.match(/\"ani\":\"sip:2222\@QA_test_server\"/)
                        .and.to.match(/\"dnis\":\"sip:2222\@QA_test_server\"/)
                        .and.to.match(/\"externalSessionID\":\"0a0325e7_00004175_528d983c_0055_0001\"/)
                        .and.to.match(/\"escalated\":false/)
                        .and.to.match(/\"sessionStats\":/)
                        .and.to.match(/\{\"stepTimes\":/)
