/**
 TC1019: Verify that Live Assist load correct application context configuration base on dnis and configurationName
 **/
//QA test case description, verification & assertion check
describe("TC1019: Verify that Live Assist load correct application context configuration base on dnis and configurationName", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
    ...
    it("TC1019: Verify that Live Assist load correct application context configuration base on dnis and configurationName", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                var testBodyParam = JSON.stringify({
                   // organizationID: "Nuance",                                 //########## special setup for this case ##################
                   // applicationID: "Test2",                                   //########## special setup for this case ##################
            ...
            function(callback){
                ...
                var testBodyParam = JSON.stringify({
                    ...
                    //"grammars":["http://mt-ray-vm06:8080/liveassist/data/vxmldemo/travel/mainmenu.grxml"]     //######## setup for this case ################
                });
            ...
            function(callback){
                ...
                test_client.get(testDebugURL + '/session/' + sessionID, function(err, res, body){
                    if(err){
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[2][1].sessionID).to.match(/session:/)
                        .and.to.match(/Nuance-MrcpApiCallSteering:.+/);                    //########### need check ##############

                    expect(result[6][0].statusCode).to.eql(200);
                    expect(result[6][1]).to.match(/\"id\":/)
                        .and.to.have.string(sessionID)
                        .and.to.match(/\"varName\":\"Nuance-MrcpApiCallSteering\"/)
                        .and.to.match(/\"varVersion\":\".+\"/)
                        .and.to.match(/\"ani\":\"sip:4627\@QA_test_server\"/)
                        .and.to.match(/\"dnis\":\"sip:4627\@127.0.0.1:5066\"/)
                        .and.to.match(/\"externalSessionID\":\"0a0325e7_000047e1_52e050ba_00b1_0001\"/)
                        .and.to.match(/\"applicationConfiguration\":\{/)
                        .and.to.match(/\"id\":\"3\"/)                                         //########### need check ##############
                        .and.to.match(/\"name\":\"Nuance-MrcpApiCallSteering\"/)               //########### need check ##############
                        .and.to.match(/\"applicationContextConfiguration\":\{/)
                        .and.to.match(/\"id\":\"1\"/)                                           //########### need check ##############
                        .and.to.match(/\"name\":\"mainmenu\"/);                                  //########### need check ##############


                    //"applicationConfiguration":{"id":"0","name":"default","organizationIDApplicationIDList":null,"dnisList":null,"contextList":null
