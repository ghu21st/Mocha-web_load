/**
 TC1005: Verify that session storage debug API get session ids
 **/
//QA test case description, verification & assertion check
describe("TC1005: Verify that session storage debug API get session ids", function () {
    //QA test setup ----------------------------------------------
...
//                            console.log('\ndeleted module:' + key);
            ...
            if(err){
                console.log('\nWarning happened from the case afterEach block!\n');
            }
            //calculate test elapse
            test_endTime = new Date();
            test_elapse = test_endTime - test_startTime;
    ...
    it("TC1005: Verify that session storage debug API get session ids", function (done) {
        ...
        async.series([
            ...
            function(callback){
                ...
                test_client.get(testBaseURL + '/sessions/ids', function(err, res, body){
                    if(err){
                        err_msg='Error found from http get request on /sessions' + err;
                        err_array.push(err_msg);
                        console.log(err_msg, err);
            ...
            function(err, result){
                ...
                try{
                    ...
                    //verify the case result
                    expect(result[5][0].statusCode).to.eql(200);
                    expect(ret_result).to.have.string(sessions[0]);
                    expect(ret_result).to.have.string(sessions[1]);
